...
<template>
    <article :id="( post_id(this_post)) " :class="( post_classes(this_post) )" v-if="(this_post.id > 0)">
        ...
        <div>{{this_post.content.rendered}}</div>
        <h1 style="color:red;">Hot reloading or not</h1>
        <p>{{this_post.featured_image.url[0]}}</p>
        <a v-bind:href=this_post.link >{{this_post.title.rendered}}</a>
        <img v-bind:src=this_post.featured_image.url[0] alt="">
        <div v-bind:style="{'backgroundImage' : 'url('+this_post.featured_image.url[0]+')'}"></div>
